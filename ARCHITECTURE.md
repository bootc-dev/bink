...
│  │                                                               │  │
│  │  ┌─────────────────────────────┐                              │  │
│  │  │ Container: k8s-dev-haproxy  │                              │  │
│  │  │ HAProxy TCP load balancer   │──── host:random-port ←→ 6443 │  │
│  │  │  → node1:6443               │                              │  │
│  │  │  → node2:6443 (if present)  │                              │  │
│  │  └─────────────────────────────┘                              │  │
│  │                                                               │  │
│  │  ┌─────────────────────────────┐                              │  │
...
Each cluster has a dedicated HAProxy container (`k8s-<cluster>-haproxy`) that load-balances API server traffic across all control-plane nodes. This eliminates the single point of failure where API access was previously tied to node1's published port.

The HAProxy container runs the `haproxy:lts-alpine` image on the same Podman bridge network as the node containers. It operates in TCP mode, forwarding connections to control-plane nodes' port 6443 via their bridge IPs. Health checks (`GET /healthz` over TLS) run every 3 seconds; a backend is marked down after 3 consecutive failures.

The kubeconfig generated by `bink api expose` points to the HAProxy's published host port (`https://localhost:<haproxy-port>`), so API access is transparent to the client. When a control-plane node goes down, HAProxy stops routing to it and directs traffic to the remaining healthy nodes.

Lifecycle:
- `bink cluster start` creates the HAProxy container with node1 as the initial backend
- `bink node add --role control-plane` recreates the HAProxy container with the new node added as a backend
- `bink cluster stop` removes the HAProxy container along with node containers (discovered via `bink.cluster-name` label)

HA requires at least 3 control-plane nodes so that etcd maintains quorum (2/3) when one node fails. With only 2 control-plane nodes, losing one causes etcd quorum loss and the remaining API server becomes unhealthy.
...
The API access path from the host: `host:random-port` → `HAProxy:6443` → `node-container:6443` (passt) → `VM:6443`.
...
Host
  │
  ├── localhost:<random-port> ──→ HAProxy:6443 ──→ node:6443 (passt) ──→ VM:6443  [API]
  │
...
10. Create the HAProxy load balancer container with node1 as the initial backend
...
├── k8s-dev-haproxy (LB)              ├── k8s-staging-haproxy (LB)
