...
var _ = Describe("Multi-Node Clusters", func() {
	...
	It("should add worker nodes and scale the cluster", Serial, func() {
		...
		node2, err := kubeClient.CoreV1().Nodes().Get(context.Background(), "node2", metav1.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		hasWorkerRole := false
		for key := range node2.Labels {
	...
	// Pending: kubeadm requires controlPlaneEndpoint in ClusterConfiguration to join
	// additional control-plane nodes. Enable once bink sets controlPlaneEndpoint in
	// internal/cluster/init.go kubeadmConfigTemplate.
	PIt("should add a control-plane node for HA configuration", func() {
		By("Creating a single-node cluster")
		helpers.CreateCluster(clusterName)

		By("Adding a control-plane node")
		...
		By("Verifying both nodes are control-plane in Kubernetes")
		helpers.WaitForNodeReady(kubeClient, "node2", 5*time.Minute)
		nodes, err := kubeClient.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{})
		Expect(err).ToNot(HaveOccurred())
		...
		By("Verifying both nodes are Ready")
		for _, node := range nodes.Items {
			ready := false
			for _, cond := range node.Status.Conditions {
				if cond.Type == "Ready" && cond.Status == "True" {
		...
		By("Verifying etcd is running on the new control-plane node")
		Eventually(func() bool {
			...
			for _, pod := range pods.Items {
				if pod.Name == "etcd-node2" {
					return true
				}
			}
			return false
		}, 3*time.Minute, 10*time.Second).Should(BeTrue(), "etcd-node2 should be running")

		By("Verifying node list shows both nodes")
		listCmd := helpers.BinkCmd("node", "list", "--cluster-name", clusterName)
		listSession := helpers.RunCommand(listCmd)
		listOutput := string(listSession.Out.Contents())
		Expect(listOutput).To(ContainSubstring("node1"), "node list should contain node1")
